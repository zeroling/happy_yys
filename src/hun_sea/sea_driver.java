...
public class sea_driver {
    public static void open(WinDef.HWND hwnd, double suofang, int time, int times) throws Exception {
        ...
        while(test3<0.8)
        {
            ...
            test3= matchop.doone(hwnd,"cmp","hunsea","open","tiaozhan");
        }
        while(flags<5&&test3>0.8)
        {
            ...
            for(int go=0;go<3;go++)
            {
                ...
                ifin=matchop.dotwo(hwnd,"cmp","hunsea","flag","flag");
                if(ifin>0.8&&tt!=0)
                {
                    System.out.println("成功进入永生之海");
                    tt=0;
                }
            ...
            {
                //悬赏处理
                test3=matchop.dotwo(hwnd,"cmp","hunsea","open","tiaozhan");
            }
        }
        /**
         * 第三次间隔为本局的总时长减去大约20秒;
         */
        r.delay(time-18000);//暂未统计游戏时长;
        /**
         * 这里是点击胜利到弹出达摩的时长,有一个2s左右的随机;
         */
        if(matchop.doone(hwnd,"cmp","hunsea","victory","victory")>0.8)
        {
        ...
        r.delay(1750);
        if(matchop.doone(hwnd,"cmp","hunsea","jingbi","jingbi")>0.8)
        {
...
//            Backclick.doMouseByPostMessage(hwnd,x2,y2+(int)(170*maining.suofang));//金币进行偏移
            if(matchop.doone(hwnd,"cmp","hunsea","jingbi","damo")>0.8)
            {
                r.delay( Aboutrandom.test(times));
                int x2=(int)(Aboutrandom.cd(Aboutopencv.shotx, Aboutopencv.imagex)*suofang);
                int y2=(int)(Aboutrandom.cd(Aboutopencv.shoty, Aboutopencv.imagey)*suofang);
